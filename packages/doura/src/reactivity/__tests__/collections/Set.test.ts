...
describe('reactivity/collections', () => {
  ...
  describe('Set', () => {
    ...
    it('should observe for of iteration', () => {
      let dummy
      const set = draft(new Set() as Set<number>)
      effect(() => {
        dummy = 0
        for (let num of set) {
          dummy += num
    ...
    it('should observe entries iteration', () => {
      let dummy
      const set = draft(new Set<number>())
      effect(() => {
        dummy = 0
    ...
    it('should not observe raw data', () => {
      let dummy
      const set = draft(new Set())
      effect(() => (dummy = toBase(set).has('value')))
    ...
    it('should not observe raw iterations', () => {
      ...
      effect(() => {
        dummy = 0
        for (let [num] of toBase(set).entries()) {
          dummy += num
        }
        for (let num of toBase(set).keys()) {
          dummy += num
        }
        for (let num of toBase(set).values()) {
          dummy += num
        }
        toBase(set).forEach((num) => {
          dummy += num
        })
        for (let num of toBase(set)) {
          dummy += num
        }
      })
    ...
    it('should not be triggered by raw mutations', () => {
      ...
      expect(dummy).toBe(false)
      toBase(set).add('value')
      expect(dummy).toBe(false)
      dummy = true
      toBase(set).delete('value')
      expect(dummy).toBe(true)
      toBase(set).clear()
      expect(dummy).toBe(true)
    })
    ...
    it('should track deletion of draft entries in raw set', () => {
      const raw = new Set()
      const entry = draft({})
      raw.add(entry)
      const set = draft(raw)
    ...
    it.skip('should warn when set contains both raw and draft versions of the same object', () => {
      const raw = new Set()
      const rawKey = {}
      const key = draft(rawKey)
      raw.add(rawKey)
      raw.add(key)
      const set = draft(raw)
      set.delete(key)
      expect(`Reactive Set contains both the raw and draft`).toHaveBeenWarned()
    })

    it('thisArg', () => {
      const raw = new Set(['value'])
      const proxy = draft(raw)
      const thisArg = {}
      let count = 0
    ...
    it('should trigger Set.has only once for non-draft keys', () => {
