...
describe('reactivity/collections', () => {
  ...
  describe('Map', () => {
    ...
    it('should not observe raw data', () => {
      let dummy
      const map = draft(new Map())
      effect(() => (dummy = toBase(map).get('key')))
    ...
    it('should not pollute original Map with Proxies', () => {
      const map = new Map()
      const observed = draft(map)
      const value = draft({})
      observed.set('key', value)
      expect(map.has('key')).toBe(false)
      expect(observed.get('key')).toBe(value)
    })

    it('should return observable versions of contained values', () => {
      const observed = draft(new Map())
      const value = {}
      observed.set('key', value)
      const wrapped = observed.get('key')
      expect(isDraft(wrapped)).toBe(true)
      expect(toBase(wrapped)).toBe(value)
    })
    ...
    it('should observe nested values in iterations (forEach)', () => {
      const map = draft(new Map([[1, { foo: 1 }]]))
      let dummy: any
      effect(() => {
        dummy = 0
        map.forEach((value) => {
    ...
    it('should observe nested values in iterations (entries)', () => {
      const key = {}
      const map = draft(new Map([[key, { foo: 1 }]]))
      let dummy: any
      effect(() => {
        dummy = 0
        for (const [key, value] of map.entries()) {
          key
          expect(isDraft(key)).toBe(false)
          expect(isDraft(value)).toBe(true)
          dummy += value.foo
        }
      })
    ...
    it('should not be trigger when the value and the old value both are NaN', () => {
      const map = draft(new Map([['foo', NaN]]))
    ...
    it('should work with draft keys in raw map', () => {
      const raw = new Map()
      const key = draft({})
      raw.set(key, 1)
      const map = draft(raw)
    ...
    it('should track set of draft keys in raw map', () => {
      const raw = new Map()
      const key = draft({})
      raw.set(key, 1)
      const map = draft(raw)
    ...
    it('should track deletion of draft keys in raw map', () => {
      const raw = new Map()
      const key = draft({})
      raw.set(key, 1)
      const map = draft(raw)
    ...
    it.skip('should warn when both raw and draft versions of the same object is used as key', () => {
      const raw = new Map()
      const rawKey = {}
      ...
      const map = draft(raw)
      map.set(key, 2)
      expect(`Reactive Map contains both the raw and draft`).toHaveBeenWarned()
    })
    ...
    it('should trigger Map.has only once for non-draft keys', () => {
