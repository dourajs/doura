import { defineModel, QueryHandle, QueryCtx, OnDataCtx } from 'doura'
...
const userModel = defineModel({
  ...
  queries: {
    ...
    fetchUser: (ctx, args: { id: string }) => {
      expectType<QueryCtx>(ctx)
      return Promise.resolve<User>({ id: args.id, name: 'User ' + args.id })
    },
    ...
    fetchUserSpec: {
      ...
      fn: (ctx, args: { id: string }) => {
        expectType<QueryCtx>(ctx)
        return Promise.resolve<User>({ id: args.id, name: 'Spec ' + args.id })
      },
      onData: (
        ctx: OnDataCtx<{ count: number }, { id: string }>,
        data: User
      ) => {
...
export function TestUseModelQueryTypes() {
  ...
  expectType<QueryHandle<void, User[]>>(api.fetchList)
  expectType<QueryHandle<{ id: string }, User>>(api.fetchUser)
  expectType<QueryHandle<{ id: string }, User>>(api.fetchUserSpec)
  ...
  expectType<User | undefined>(api.fetchUser.getData({ id: '1' }))
  expectType<boolean>(api.fetchUser.isFetching({ id: '1' }))
  expectType<Promise<User>>(api.fetchUser.fetch({ id: '1' }))
  api.fetchUser.setData({ id: '1' }, { id: '1', name: 'Alice' })

  // invalidate / reset accept either specific args or "all entries".
  api.fetchUser.invalidate()
  api.fetchUser.invalidate({ id: '1' })
  api.fetchUser.reset()
  api.fetchUser.reset({ id: '1' })
  ...
  api.fetchList.getData({ id: '1' })

  // Explicit shape check of the conditional setData signature.
  const setUser: (args: { id: string }, data: User) => void =
    api.fetchUser.setData
  expectType<(args: { id: string }, data: User) => void>(setUser)
