// @ts-nocheck
/**
 * Benchmark: batch mutation (10% of elements) on array — slow path (snapshot Proxy).
 *
 * Mirrors array-batch.ts but uses the ModelInternal pattern:
  ...
  libs: [
    ...
    },
    {
      ...
      fn: `
        ...
        for (let index = 0; index < size * 0.1; index++) {
          stateRef.value[index].value = ++__iter;
        }
